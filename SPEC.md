...
| 形态 | 角色 | 关键能力 |
|---|---|---|
| **macOS 菜单栏 App**（`Sources/HengqinTracker`） | 数据采集 + 可视化 + 编辑 + 跨设备同步 | 年/月热力图；手动标记；多坐标档案 + 地图选点；导入导出；iCloud KVS 跨设备同步 |

历史背景：早期版本曾配套 Chrome 扩展作为自动数据来源（2026-05-20 之后已下线，相关源码已从仓库移除）。当前唯一支持形态为 macOS App，数据持久化在本地 Application Support + 可选的 iCloud KVS。
...
macOS 端支持**多坐标档案**，便于追踪除横琴外其他地点的驻留（例：在深圳办事处、在老家等）。
...
└── records/
    ├── <profileId-1>.json              # 该 profile 的所有 DayRecord（dict { "day_2026-01-01": {...} } 形式）
    └── <profileId-2>.json
```
...
- JSON 格式与历史 Chrome 扩展时代保持兼容，便于老用户迁移
