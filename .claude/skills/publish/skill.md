...
- 提交时附带 `Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>`

### 4. 打包 zip

- 输出路径：`/Users/bytedance/Documents/workspace/hengqin/hengqin-tracker.zip`
- **必须先删除旧 zip**（`rm -f`），因为 `zip -r` 是追加模式，不删会导致多份 manifest 共存
- 排除项：`.git/*`、`.claude/*`、`.DS_Store`、`*.zip`、`__MACOSX/*`
- 命令：`rm -f <输出路径> && zip -r <输出路径> . -x '.git/*' '.claude/*' '.DS_Store' '*.zip' '__MACOSX/*'`
