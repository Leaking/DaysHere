...
- **`promote`**：将 decisions.md 的精华提炼后合并进 AGENTS.md，使 AGENTS.md 反映最新架构状态
...
## 模式二：提炼并更新 AGENTS.md（参数：promote）

### 核心指令

将 decisions.md 中积累的决策记录提炼压缩，合并进 AGENTS.md，使 AGENTS.md 始终反映项目的最新架构状态。

### 执行流程

1. 读取 AGENTS.md 和 decisions.md（必须两个都存在才能继续）
2. 分析 decisions.md 中所有变更记录，识别：
   - 与 AGENTS.md 现有内容**冲突或过时**的描述 → 需要替换
   - AGENTS.md **缺失但重要**的新架构信息 → 需要补充
   - 仅属于历史过程、不影响当前架构的内容 → 忽略
3. 生成对 AGENTS.md 的最小化修改方案（diff 风格展示），原则：
   - **保持 AGENTS.md 精简**：不堆砌历史，只留对开发者有指导意义的内容
   - **不改变 AGENTS.md 整体结构**：在对应章节就地更新，而非追加
   - **语言风格一致**：延续 AGENTS.md 已有的简洁技术风格
4. 展示变更 diff，询问："是否批准更新 AGENTS.md？（yes / no / edit）"
5. 得到批准后写入 AGENTS.md
